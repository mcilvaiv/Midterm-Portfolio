...
## In this plot, I wanted to see the correlation between my friends' characteristics. To do this I first selected only the numerical variables from the data I created. Next, I uses ggcorplot and loaded in that library to create the correlation matrix. To make it look nicer, I used options to modify what the matrix looks likes. Then, I use cor_mat to find p-values for the correlation. Where the correlation is >0.05, an X is displayed on the correlation sqaure.
```{r}
# Correlation 
